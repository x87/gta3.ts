...
async function body() {
    ...
    while (!Streaming.HasSpecialCharacterLoaded(1) || !Streaming.HasSpecialCharacterLoaded(2) || !Streaming.HasModelLoaded(CAR_IDAHO)) {
        await asyncWait(0);
    }
    ...
    $.blip2_jm1 = Blip.AddSpriteForCoord(1282.0, -104.0, -100.0, RADAR_SPRITE_BOMB);
    $.blip4_jm1 = Blip.AddSpriteForCoord(925.0, -359.5, -100.0, RADAR_SPRITE_SPRAY);
    ...
    while (!Streaming.HasModelLoaded(CAR_IDAHO) || !Streaming.HasSpecialCharacterLoaded(3)) {
        await asyncWait(0);
    }
    ...
    $.mike_car = Car.Create(CAR_IDAHO, 1336.2, -460.8, -100.0);
    $.mike_car.makeABitStronger(true);
    $.mike_car.lockDoors(CARLOCK_FORCE_SHUT_DOORS);
    $.mike_car.setHeading(90.0);
    $.mike_car.setCanRespray(OFF);
    ...
    Game.SetFreeBombs(ON);

    Go_to_8Balls: while (true) {
        $.flag_car_blip_displayed_jm1 = 1 /* TRUE */;

        while (!$.mike_car.isArmedWithBomb(CARBOMB_ONIGNITION) && !$.mike_car.isArmedWithBomb(CARBOMB_ONIGNITIONACTIVE)) {
            await asyncWait(0);
            if (Car.IsDead($.mike_car)) {
                Text.PrintNow('WRECKED', 5000, 1);
                FAIL("mission_joey1_failed");
            ...
            if ($.player.isInCar($.mike_car)) {
                if ($.flag_car_blip_displayed_jm1 == 1 /* TRUE */) {
                    if (!$.mike_car.isArmedWithBomb(CARBOMB_ONIGNITIONACTIVE) || !$.mike_car.isArmedWithBomb(CARBOMB_ONIGNITION)) {
                        $.blip2_jm1.remove();
                        $.blip2_jm1 = Blip.AddSpriteForCoord(1282.0, -104.0, -100.0, RADAR_SPRITE_BOMB);
                        $.blip1_jm1.remove();
                    }
            ...
            if (!$.mike_car.isHealthGreater(700) || $.mike_car.isVisiblyDamaged()) {
                ...
                $.blip1_jm1.remove();
                $.blip1_jm1 = Blip.AddForCar($.mike_car);
                $.blip1_jm1.changeDisplay(BLIP_ONLY);
                $.flag_car_blip_displayed_jm1 = 1 /* TRUE */;
    ...
    one_before_joey_label1: Game.SetFreeBombs(OFF);
    ...
    joey_label1: while (true) {
        ...
        cars_rigged: while (true) {
            ...
            if (!$.mike_car.isArmedWithBomb(CARBOMB_ONIGNITIONACTIVE)) {
    ...
    $.lips = Char.Create(PEDTYPE_SPECIAL, PED_SPECIAL3, 1325.5, -452.5, -100.0);
    $.lips.setHeading(270.0);

    Camera.SetFixedPosition(1337.814, -468.631, 49.774, 0.0, 0.0, 0.0);
    Camera.PointAtPoint(1337.053, -468.016, 49.982, JUMP_CUT);
    ...
    if (!Car.IsDead($.mike_car)) {
        Camera.PointAtCar($.mike_car, FIXED, JUMP_CUT);
    }
...
async function cleanup() {
    ...
    Hud.ClearTimer($.$id.countdown_jm1);
    if (!Car.IsDead($.mike_car)) {
        $.mike_car.setCanRespray(ON);
        $.mike_car.lockDoors(CARLOCK_UNLOCKED);
        $.mike_car.makeABitStronger(false);
    }
    Game.SetFreeBombs(OFF);
    Mission.Finish();
}
