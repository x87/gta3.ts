...
(async () => {
    await run_on_newgame(async () => {
        ...
        verbose('[+] import script initialized variables');
    });
    ...
    import1_loop(); // START_NEW_SCRIPT import1_loop
    military_crane_loop(); // START_NEW_SCRIPT military_crane_loop
    import2_loop(); // START_NEW_SCRIPT import2_loop
    check_packages(); // START_NEW_SCRIPT check_packages

    mission_import_start_inner: while (true) {
        await asyncWait(500);
                            ...
                            if ($.player.isInArea2D(1486.9, -686.2, 1524.1, -666.8, false /* FALSE */)) {
                                ...
                                if ($.import_car_been_created_before == 0) {
                                    if ($.securicar_pickup.hasBeenCollected()) {
                                        $.import_car_type = 118;
                                        await load_vehicle(); // SCM GOTO → load_vehicle
                                        continue pick_up_pick_ups;
                                    }
...
async function military_crane_loop() {
    //	Should be called in mission_import_start
    // SCRIPT_NAME M_CRANE
    military_crane_loop_inner: while (true) {
        await asyncWait(500);
        ...
        if ($.player.isPlaying()) {
            if ($.player.isInArea2D(1548.1, -745.5, 1583.0, -675.1, false /* FALSE */)) {
                if (Crane.HasMilitaryCollectedAllCars()) {
                    ...
                    if ($.military_car_been_created_before == 0) {
                        if ($.copcar_pickup.hasBeenCollected()) {
                            ...
                            await load_vehicle3(); // SCM GOTO → load_vehicle3
                            continue military_crane_loop_inner;
                        }
                ...
                if ($.create_military_pickups == 1) {
                    await mission_remove_pickups_military(); // SCM GOSUB mission_remove_pickups_military
                    $.create_military_pickups = 0;
                }
                if ($.player.isPlaying()) {
                            ...
                            if ($.player.isInArea2D(-1117.4, 158.1, -1098.0, 121.5, false /* FALSE */)) {
                                ...
                                if ($.import_car_been_created_before2 == 0) {
                                    if ($.sentinet_pickup.hasBeenCollected()) {
                                        $.import_car_type2 = 95;
                                        await load_vehicle2() // SCM GOTO → load_vehicle2
                                        continue pick_up_pick_ups2;
                                    }
