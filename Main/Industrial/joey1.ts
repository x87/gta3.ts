...
async function cars_rigged() {
  ...
  while (!($.mike_car.isStoppedInArea3D(1339.6, -459.5, 49.0, 1332.8, -462.8, 53.0, true /* TRUE */)) || $.player.isInArea3D(1306.0, -482.0, 49.0, 1350.0, -444.0, 59.0, false /* FALSE */) || !($.player.isInArea3D(1306.0, -484.0, 49.0, 1370.0, -434.0, 69.0, false /* FALSE */))) {
    await asyncWait(0);
    if (Car.IsDead($.mike_car)) {
      Text.PrintNow("WRECKED", 5000, 1);
      // SCM GOTO → mission_joey1_failed (not lowered; manual jump required)
