...
async function mission_start_frank4() {
  ...
  while (!(Streaming.HasSpecialCharacterLoaded(1)) || !(Streaming.HasModelLoaded(hier`cutobj01`)) || !(Streaming.HasModelLoaded(hier`cutobj02`)) || !(Streaming.HasModelLoaded(ped`GANG_MAFIA_B`))) {
    await asyncWait(0);
  }
  ...
  Text.PrintNow("FM4_A", 10000, 1);
  while ($.cs_time < 5260) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_B", 10000, 1);
  while ($.cs_time < 9273) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_C", 10000, 1);
  while ($.cs_time < 12861) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_D", 10000, 1);
  while ($.cs_time < 15447) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_E", 10000, 1);
  while ($.cs_time < 18080) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_F", 10000, 1);
  while ($.cs_time < 22832) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_H", 10000, 1);
  while ($.cs_time < 24908) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  ...
  if ($.barriers_been_added == 0) {
    $.subway_gate_suburban1 = Object.CreateNoOffset(subwaygate, -672.0, -760.0, 8.3);
    $.subway_gate_suburban1.DontRemove();
    $.subway_gate_suburban1.SetHeading(270.0);
    $.subway_gate_suburban2 = Object.CreateNoOffset(subwaygate, -672.0, -768.9, 8.3);
    $.subway_gate_suburban2.DontRemove();
    $.subway_gate_suburban2.SetHeading(270.0);
    $.tunnel_gate_suburban = Object.CreateNoOffset(tunnelentrance, 533.3, 97.0, -21.3);
  ...
  while (!($.player.LocateStoppedInCar3D(827.3, -1084.3, 6.8, 5.0, 5.0, 4.0, 1 /* TRUE */))) {
    await asyncWait(0);
    if (Car.IsDead($.bomb_car) && $.car_dead_once == 0) {
      $.blip1_fm4.Remove();
      $.car_dead_once = 1;
    }
    if ($.player.LocateOnFoot3D(827.3, -1084.3, 6.8, 5.0, 5.0, 4.0, 0 /* FALSE */)) {
...
async function on_foot_frank4() {
  ...
  Text.PrintNow("FM4_2", 10000, 1);
  while ($.cs_time < 3275) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_3", 10000, 1);
  while ($.cs_time < 6479) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_4", 10000, 1);
  while ($.cs_time < 9000) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_4B", 10000, 1);
  while ($.cs_time < 11356) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_5", 10000, 1);
  while ($.cs_time < 13307) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_6", 10000, 1);
  while ($.cs_time < 16285) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_6B", 10000, 1);
  while ($.cs_time < 19438) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_7", 10000, 1);
  while ($.cs_time < 23340) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_8", 10000, 1);
  while ($.cs_time < 24733) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("FM4_9", 10000, 1);
  while ($.cs_time < 30156) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  ...
  $.maria.SetOnlyDamagedByPlayer(1 /* TRUE */);
  $.maria.SetHeading(17.0);
  $.maria.SetStaysInCurrentLevel(0 /* FALSE */);
  $.maria.SetIgnoreLevelTransitions(1 /* TRUE */);
  $.asuka = Char.Create(21 /* PEDTYPE_SPECIAL */, ped`SPECIAL3`, 836.6, -1114.6, 1.8);
  $.asuka.SetOnlyDamagedByPlayer(1 /* TRUE */);
  $.asuka.SetHeading(178.0);
  $.asuka.SetStaysInCurrentLevel(0 /* FALSE */);
  $.asuka.SetIgnoreLevelTransitions(1 /* TRUE */);
  ...
  while (!($.player.IsInArea3D(565.3, -678.4, -2.0, 575.3, -688.4, 4.0, 1 /* TRUE */)) || !($.maria.IsInArea3D(565.3, -678.4, -2.0, 575.3, -688.4, 4.0, 0 /* FALSE */)) || !($.asuka.IsInArea3D(565.3, -678.4, -2.0, 575.3, -688.4, 4.0, 0 /* FALSE */))) {
    await asyncWait(0);
    if (Car.IsDead($.boat_mar)) {
      // SCM GOTO → mission_frank4_failed (not lowered; manual jump required)
      throw new Error("unresolved GOTO mission_frank4_failed"); // fallback: would break linear control flow
  ...
  if (!(Car.IsDead($.boat_mar))) {
    if ($.player.IsInAnyCar()) {
      $.player.WarpFromCarToCoord(515.1, -650.9, 16.0);
    }
  }
  ...
  Text.PrintNow("LRQC_1", 5000, 1);
  while ($.cs_time < 14476) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("LRQC_2", 5000, 1);
  while ($.cs_time < 16021) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("LRQC_3", 5000, 1);
  while ($.cs_time < 17946) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("LRQC_4", 5000, 1);
  while ($.cs_time < 21438) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("LRQC_5", 5000, 1);
  while ($.cs_time < 23312) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
  Text.PrintNow("LRQC_6", 5000, 1);
  while ($.cs_time < 25177) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }
...
async function mission_frank4_passed() {
  ...
  Path.SwitchRoadsOn(619.6, -911.5, 45.0, 834.3, -954.5, 32.0);
  Path.SwitchRoadsOn(659.6, 200.0, -20.0, 945.8, 147.5, 5.0);
  Path.SwitchRoadsOn(529.6, 106.5, -30.0, 581.4, 65.7, 0.0);
