...
async function mission_start_toni3() {
  ...
  */
        ...
        while (
            !Streaming.HasModelLoaded(car`STRETCH`) ||
            !Streaming.HasModelLoaded(car`BELLYUP`) ||
            !Streaming.HasModelLoaded(ped`GANG_TRIAD_A`) ||
            !Streaming.HasModelLoaded(ped`GANG_TRIAD_B`)
        ) {
            await asyncWait(0)
        }
        /*
