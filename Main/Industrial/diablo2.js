...
async function mission_start_diablo2() {
  ...
  Streaming.RequestModel(car`MRWHOOPEE`);
  Streaming.RequestModel(ped`GANG_MAFIA_A`);
  Streaming.RequestModel(ped`LI_MAN1`);
  while (!(Streaming.HasModelLoaded(car`MRWHOOPEE`)) || !(Streaming.HasModelLoaded(ped`GANG_MAFIA_A`)) || !(Streaming.HasModelLoaded(ped`LI_MAN1`))) {
    await asyncWait(0);
  }
  await asyncWait(2000);
  $.briefcase_diablo2 = Pickup.Create(briefcase, 3 /* PICKUP_ONCE */, 934.9, -69.8, 8.1);
  ...
  $.icecream_van1 = Car.Create(106 /* CAR_MRWHOOPEE */, 1381.0, -382.0, -100.0);
  ...
  while (!($.icecreamvan_any.IsStoppedInArea3D(1215.9, -1128.7, 11.2, 1210.4, -1123.3, 14.2, $.blob_flag)) || !($.player.IsInCar($.icecreamvan_any)) || !($.icecreamvan_any.IsIcecreamJingleOn())) {
    ...
    if ($.icecreamvan_any.IsStoppedInArea3D(1215.9, -1128.7, 11.2, 1210.4, -1123.3, 14.2, 0 /* FALSE */) && $.player.IsInCar($.icecreamvan_any)) {
  ...
  World.ClearArea(1190.5, -1141.2, 11.6, 2.0, 1 /* TRUE */);
  $.creamed_guy1 = Char.Create(7 /* PEDTYPE_GANG_MAFIA */, ped`GANG_MAFIA_A`, 1190.5, -1141.2, 11.6);
  World.ClearArea(1192.5, -1141.2, 11.6, 2.0, 1 /* TRUE */);
  $.creamed_guy2 = Char.Create(7 /* PEDTYPE_GANG_MAFIA */, ped`GANG_MAFIA_A`, 1192.5, -1141.2, 11.6);
  World.ClearArea(1194.5, -1141.2, 11.6, 2.0, 1 /* TRUE */);
  $.creamed_guy3 = Char.Create(7 /* PEDTYPE_GANG_MAFIA */, ped`GANG_MAFIA_A`, 1194.5, -1141.2, 11.6);
  World.ClearArea(1196.5, -1141.2, 11.6, 2.0, 1 /* TRUE */);
  $.creamed_guy4 = Char.Create(7 /* PEDTYPE_GANG_MAFIA */, ped`GANG_MAFIA_A`, 1196.5, -1141.2, 11.6);
  ...
  await asyncWait(2000);
  World.ClearArea(1205.6, -1141.1, 11.6, 10.0, 1 /* TRUE */);
  ...
  while (!($.ojective_creamed_guys_passed == 4)) {
    ...
    if (!(Char.IsDead($.creamed_guy1))) {
      if ($.player.LocateAnyMeansChar3D($.creamed_guy1, 8.0, 8.0, 2.0, 0 /* FALSE */)) {
        $.creamed_guy1.SetObjKillPlayerOnFoot($.player);
        $.creamed_guy1.SetThreatSearch(0 /* THREAT_PLAYER1 */);
      }
    }
    if (!(Char.IsDead($.creamed_guy2))) {
      if ($.player.LocateAnyMeansChar3D($.creamed_guy2, 8.0, 8.0, 2.0, 0 /* FALSE */)) {
        $.creamed_guy2.SetObjKillPlayerOnFoot($.player);
        $.creamed_guy2.SetThreatSearch(0 /* THREAT_PLAYER1 */);
      }
    }
    if (!(Char.IsDead($.creamed_guy3))) {
      if ($.player.LocateAnyMeansChar3D($.creamed_guy3, 8.0, 8.0, 2.0, 0 /* FALSE */)) {
        $.creamed_guy3.SetObjKillPlayerOnFoot($.player);
        $.creamed_guy3.SetThreatSearch(0 /* THREAT_PLAYER1 */);
      }
    }
    if (!(Char.IsDead($.creamed_guy4))) {
      if ($.player.LocateAnyMeansChar3D($.creamed_guy4, 8.0, 8.0, 2.0, 0 /* FALSE */)) {
        $.creamed_guy4.SetObjKillPlayerOnFoot($.player);
        $.creamed_guy4.SetThreatSearch(0 /* THREAT_PLAYER1 */);
      }
    }
    if (!(Char.IsDead($.creamed_guy1))) {
...
async function mission_cleanup_diablo2() {
  ...
  Streaming.MarkModelAsNoLongerNeeded(car`MRWHOOPEE`);
  Streaming.MarkModelAsNoLongerNeeded(ped`GANG_MAFIA_A`);
  Streaming.MarkModelAsNoLongerNeeded(ped`LI_MAN1`);
