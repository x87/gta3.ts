...
async function mission_start_diablo4() {
  ...
  World.ClearArea(918.2, -269.7, 5.0, 5.0, 1 /* TRUE */);
  ...
  if ($.diablo_collect_porn_van.Locate2D(918.2, -269.7, 10.0, 10.0, 0 /* FALSE */)) {
  ...
  while (!(Char.IsDead($.porn_man))) {
    ...
    if (!(Char.IsDead($.porn_man))) {
      if ($.player.LocateAnyMeansChar2D($.porn_man, 30.0, 30.0, 0 /* FALSE */) && $.found_perv_before == 0) {
  ...
  while (!($.diablo_collect_porn_van.IsStoppedInArea3D(976.5, -422.8, 14.5, 970.8, -433.4, 16.9, 1 /* TRUE */))) {
    await asyncWait(0);
    if (Car.IsDead($.diablo_collect_porn_van)) {
      Text.PrintNow("WRECKED", 5000, 1);
      // SCM GOTO → mission_diablo4_failed (not lowered; manual jump required)
  ...
  await asyncWait(1000);
  World.ClearArea(988.9, -444.7, 14.9, 10.0, 1 /* TRUE */);
  ...
  await asyncWait(3000);
  World.ClearArea(978.9, -444.4, 14.9, 3.0, 1 /* TRUE */);
  if (!(Char.IsDead($.pervert3))) {
  ...
  if (!(Car.IsDead($.diablo_collect_porn_van))) {
    if ($.player.IsInCar($.diablo_collect_porn_van)) {
      World.ClearArea(978.3, -442.7, 13.9, 1.0, 1 /* TRUE */);
...
async function mission_diablo4_passed() {
  ...
  $.donkeymag1 = Object.Create(donkeymag, 895.2, -308.2, 7.4);
  $.donkeymag1.DontRemove();
  $.donkeymag2 = Object.Create(donkeymag, 896.1, -308.6, 7.4);
  $.donkeymag2.DontRemove();
  $.donkeymag3 = Object.Create(donkeymag, 890.1, -309.6, 7.4);
