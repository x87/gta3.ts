...
async function body() {
    ...
    Streaming.RequestModel(PED_B_MAN1);

    Streaming.RequestModel(CAR_PERENNIAL);

    Audio.LoadMissionAudio(SfxMission.Mf1_a);

    while (!Streaming.HasModelLoaded(PED_B_MAN1) || !Streaming.HasModelLoaded(CAR_PERENNIAL) || !Audio.HasMissionAudioLoaded()) {
        await asyncWait(0);
    }

    $.car_meat1 = Car.Create(CAR_PERENNIAL, 1190.0, -796.0, 13.8);
    ...
    $.bankmanager_meat1 = Char.Create(PEDTYPE_CIVMALE, PED_B_MAN1, 1039.0, -695.0, 13.9);
    ...
    while (!$.bankmanager_meat1.locateOnFoot3D(1205.9, -805.8, 14.0, 1.0, 1.0, 1.0, false)) {
        ...
        if (TIMERB >= 20000) {
            if (!$.bankmanager_meat1.locateOnFoot3D(1205.9, -805.8, 14.0, 1.0, 1.0, 1.0, false)) {
                ...
                break; // SCM GOTO → bloke_got_stuck_meat1
            }
        }
        if ($.car_meat1.isUpsidedown() && $.car_meat1.isStopped()) {
            Text.PrintNow('UPSIDE', 5000, 1); //"The vehicle's upsidedown!"
    ...
    bloke_got_stuck_meat1: {
        Audio.LoadMissionAudio(SfxMission.Mf4_a);
        ...
        while ($.flag_car_crushed_meat1 == 0) {
            ...
            if ($.car_meat1.isCrushed()) {
                ...
                break; // SCM GOTO → mission_passed_meat1
            } else {
                if (Car.IsDead($.car_meat1)) {
                    Text.PrintNow('MEA1_2', 5000, 1); //"You idiot! The cops will find the evidence in the car."
                    FAIL('mission_failed_meat1');
...
async function onPassed() {
    ...
    // START_NEW_SCRIPT meat_mission2_loop // xxx: moved in mission monitor
}
...
async function cleanup() {
    ...
    $.bankmanager_meat1.removeElegantly();
    Streaming.MarkModelAsNoLongerNeeded(PED_B_MAN1);
